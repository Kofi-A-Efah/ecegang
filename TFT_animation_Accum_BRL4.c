...
_Accum neighbor_arr[boid_num]; 
_Accum match_factor[boid_num];
// Parameters
static int x;
static int y;
static _Accum vx;
...
//                    boid_arr[i].y < right_screen && boid_arr[i].y >= 0){
                ...
                for ( j = i + 1 + boid_arr[i].oddEven; j < boid_num; j = j + 2) { 
				  ...
				  //get distance between boid and otherboid
					...
					if ( dx < visualRange && dy < visualRange ) {
                        ...
                        if (dx > dy) {
                            tempDistance = dx + (dy >> 2);
                        }
                        else {
                            tempDistance = dy + (dx >> 2);
                        }
						...
						//alignment code
						else if (tempDistance < visualRange) {
							//update boid
           ...
           int neighbor_int = Accum2int(boid_arr[i].neighboring_boids);
           if (boid_arr[i].neighboring_boids != 0) {
               //boid_arr[i].neighboring_boids = 1/boid_arr[i].neighboring_boids;              
              boid_arr[i].neighboring_boids = neighbor_arr[neighbor_int];
           }
           
           //alignment stuff
           boid_arr[i].vx += boid_arr[i].xvel_avg * match_factor[neighbor_int];
           boid_arr[i].vy += boid_arr[i].yvel_avg * match_factor[neighbor_int];
           if (boid_arr[i].neighboring_boids > 0) {
...
//            }
            ...
            if (speed < minspeed) {
                boid_arr[i].vx = boid_arr[i].vx * 1.4;
                boid_arr[i].vy = boid_arr[i].vy * 1.4;
            }
            if (speed > maxspeed) {
                boid_arr[i].vx = boid_arr[i].vx * 0.8;
                boid_arr[i].vy = boid_arr[i].vy * 0.8;
            }           
...
void main(void) {
 //SYSTEMConfigPerformance(PBCLK);
  ...
  for (i = 1; i < boid_num; i++) {
      _Accum temp = float2Accum((1.0/i));
      neighbor_arr[i] =  temp;
      match_factor[i] =  temp*0.05; 
  }
  //rand() % 240, rand() % 320
