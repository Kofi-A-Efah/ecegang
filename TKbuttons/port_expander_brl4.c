...
#define SET_CS2    {mPORTASetBits(BIT_0);}
#define CLEAR_CS2  {mPORTAClearBits(BIT_0);}
...
void initPE() {
  ...
  volatile int spiClkDiv = 8; // 10 MHz max speed for this DAC
  mPORTBSetPinsDigitalOut(BIT_9); // use RPB9 (pin 21)
  mPORTBSetBits(BIT_9); // CS active low

  mPORTASetPinsDigitalOut(BIT_0); // use RPA0 
  mPORTASetBits(BIT_0); // CS 2 (RA0) active low  
...
void clearBits2(unsigned char addr, unsigned char bitmask){
  if (addr <= 0x15){
    unsigned char cur_val = readPE2(addr);
...
void mPortYSetPinsOut2(unsigned char bitmask){
  clearBits2(IODIRY, bitmask);
}
...
void mPortZSetPinsOut2(unsigned char bitmask){
  clearBits2(IODIRZ, bitmask);
}
...
void mPortYSetPinsIn2(unsigned char bitmask){
  setBits2(IODIRY, bitmask);
}

void mPortZSetPinsIn2(unsigned char bitmask){
  setBits2(IODIRZ, bitmask);
}
