...
#                                         (PIC gnd)
#
# There is a separate thread to handle serial communication from the PIC
# The separate thread allows the comm code to be more efficient and 
# not block the main GUI
# Two events are generated by the comm thread: 
#   'PIC_recv' just forwards the text to the multiline display
#   'PIC_cmd' allows the PIC to send data back to the GUI interface logic
...
layout = [  [sg.Text('Flock Control',  background_color=heading_color)],
            ...
            [sg.Text('   Centering Factor '),
             sg.Slider(range=(1.00,5000.00), default_value=2000, size=(22,15), key='slider3', resolution=100,
             orientation='horizontal', font=('Helvetica', 12),enable_events=True)],
            #
            ...
            [ sg.Checkbox('reset_enable', key='r_en', 
             ...
             sg.Text('sys_time', key = 'sys_time', size=[8,1], justification='right'),
             sg.Text(' ', key = 'sys_led', size=[1,1], background_color='red')
            ] 
         ]
